...
class RoutesAntFamiliares{
    static async createAntFml(form,id_paciente){
        console.log(form, 'sssssss')
        const token = await Token.getToken();
        if(!token) return {data:{success:false,msg:"no hay token"} };
        try {
            const resp = await axios.post(`${Url.urlBackEnd}/api/createAntFamiliares/${token.user.id}/${id_paciente}`, form,{         
                headers: {
                    'c_token': token.t
                }
            });           
...
export default RoutesAntFamiliares;