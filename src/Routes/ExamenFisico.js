...
class RoutesExamenFisico{
    static async createExamenFisico(form,id_paciente){
        const token = await Token.getToken();
        if(!token) return {data:{success:false,msg:"no hay token"} };
        try {
            const resp = await axios.post(`${Url.urlBackEnd}/api/createExamenFisico/${token.user.id}/${id_paciente}`, form,{                                
                headers: {
                    'c_token': token.t
                }
            });           
...
export default RoutesExamenFisico;