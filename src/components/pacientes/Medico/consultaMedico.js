...
function ConsultaMedico(props) {
    ...
    const getLisPacientes = useCallback(async () =>{
        try {
          setLoad(true);
          const resp = await MedicoRoutes.pacienteConsultaDoctor(props.id_medico,search.buscador);
    ...
    const buscar = async (e) => {
      ...
      try {
        setLoad(true);
        const resp = await MedicoRoutes.pacienteConsultaDoctor(props.id_medico,buscador);
        if(resp.data.success === false){
          setLoad(false);
          setRespErr(true);
          setTimeout(() => setRespErr(false), 5000);
    ...
    return (
        <>
                                        ...
                                        <tr key={key}>
                                            ...
                                            <div className="btn-group btn-group-sm">
                                                <button 
                                                    ...
                                                    className="btn btn-primary" data-toggle="modal" data-target="#pd1"><i className="fas fa-eye" />
                                                </button>                                
                                            </div>
            ...
            <ModalLarge title={`Paciente: ${selected.nombres} ${selected.apellidos}. Edad: ${selected.edad}`} idModal="pd1" large="xl">
